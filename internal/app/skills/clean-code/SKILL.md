...
Before marking work done: `go build ./...` passes, `go test ./...` passes, `golangci-lint run` clean.
...
- `coding-standards` - Specific Go quality rules for linters and SonarCloud
...
- `modular-design` - Testability and composability lens on clean code units
- `tdd-first` - Enforces test-first discipline; clean-code applies during the refactor phase
