...
func NewWithOptions(cfg *config.AppConfig, opts NewOptions) (*App, error) {
	...
	if err := ValidateDiscoveredGateReferences(context.Background(), cfg, swarmRegistry); err != nil {
		slog.Error("swarm registry references undiscovered ext gate kinds", "err", err)
		return nil, fmt.Errorf("validating discovered ext gate references: %w", err)
	}
...
// RegisterProvidersForTest is a test helper that delegates to
// providers.Build for testing.
//
// Expected:
...
// RegisterProvidersWithFailuresForTest is a test helper that delegates to
// providers.BuildWithFailures so tests can assert which providers failed to
...
//
// Side effects:
//   - Same as providers.BuildWithFailures: reads environment variables and
//     logs warnings.
func RegisterProvidersWithFailuresForTest(
	cfg *config.AppConfig,
) (*provider.Registry, *ollama.Provider, map[string]error) {
	return providers.BuildWithFailures(cfg)
}
...
// SetAutoresearchRunner injects the AutoresearchRunner implementation used
// by wireDelegateToolIfEnabled to register the autoresearch_run engine tool.
