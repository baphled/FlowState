...
var _ = Describe("Coordination tool wiring", func() {
	...
	It("exposes the canonical name + description", func() {
		...
		Expect(coordTool.Description()).To(Equal(
			"Inter-agent handoff store for delegation chains: publish contracted outputs for other agents and read theirs. Not general storage and never user-facing output — write user-requested content to files with the write tool. Never access the backing coordination.json file directly."))
	})
