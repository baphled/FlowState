...
- **Signal completion**: When the final item flips to `completed`, close the loop with the `EXECUTION COMPLETE` signal and the summary already required by the execution protocol. Then call `todo_clear` to retire the finished list — this does not affect session state and allows a fresh `todowrite` for the next plan.
