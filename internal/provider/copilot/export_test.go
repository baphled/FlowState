...
// SetBaseURLForTest overrides the Copilot chat base URL so a spec can point the
// streaming client (built via buildClient) at a local test server.
func (p *Provider) SetBaseURLForTest(u string) {
	p.baseURL = u
}
