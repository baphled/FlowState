...
func (tm *TokenManager) RefreshNow(ctx context.Context) error {
	...
	if tm.refresher == nil {
		tm.consecutiveFailures++
		return fmt.Errorf("anthropic token: no refresher configured")
	}
	...
	// Persist the refreshed tokens so the next process start picks
	// them up.
	if perr := tm.persistTokens(); perr != nil {
		slog.Warn("anthropic token RefreshNow: persist failed; in-memory tokens still valid",
			"error", perr)
	}
	return nil
}
