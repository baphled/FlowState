...
var _ = Describe("buildTodoContinuationMessage", func() {
	Context("when all items are pending", func() {
		It("returns the pending-only continuation message", func() {
			items := []todo.Item{
	...
	Context("when items are in_progress and pending", func() {
		It("returns the active-task continuation message with queued tasks", func() {
			items := []todo.Item{
			...
			Expect(msg.Content).To(ContainSubstring("You must complete or cancel the active task now"))
	...
	Context("when the only item is in_progress", func() {
		It("returns the active-task continuation message without queued tasks", func() {
			items := []todo.Item{
