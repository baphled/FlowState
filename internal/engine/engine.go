...
// applyFactRecall injects recalled long-term facts into the outgoing
// message window for the supplied session, prefixing them to the live
// messages so the provider sees durable context on this turn.
//
// Expected:
//   - factService, a non-empty sessionID and at least one message are
//     required; otherwise the window is returned unchanged.
//   - Recall failures are logged and degrade to the unmodified window.
//
// Returns:
//   - The (possibly fact-prefixed) message slice for this turn.
//
// Side effects:
//   - Emits a warning log when recall fails or a fact block is skipped.
...
// emitMidToolLoopRefresh rebuilds the context window mid-tool-loop and
// streams a context refresh chunk so long-running tool loops keep the
// provider's window coherent.
//
// Expected:
//   - A non-nil engine with a store and non-empty sessionID; otherwise
//     the refresh is skipped.
//   - liveMessages carries the current turn's transcript so far.
//
// Returns:
//   - true when a refresh chunk was assembled and emitted, false when
//     the refresh was skipped or assembly failed.
//
// Side effects:
...
// preferredProviderModel returns the provider and model name of the
// first manifest preferred model entry.
//
// Expected:
//   - manifest may be nil or declare no preferred models; both cases
//     are tolerated rather than rejected.
//
// Returns:
//   - The provider and model of the first preferred entry, or two
//     empty strings when no preference exists.
//
// Side effects:
//   - None.
...
// CompressionMetrics returns a snapshot of the engine's cumulative
// compression counters across all sessions.
//
// Expected:
//   - The engine need not have compacted anything yet; a fresh engine
//     yields the zero value.
//
// Returns:
//   - A copy of the cumulative CompressionMetrics aggregate.
//
// Side effects:
//   - None. The returned value is a copy; mutating it leaves the
//     engine's live counters untouched.
...
// buildResultInputs bundles the parameters the context-window assembly
// path passes from Stream into the WindowBuilder, keeping the call
// sites readable and the assembly routine's signature stable.
