...
// NotificationEvent specs — restored after the test-file consolidation
// accident discarded internal/plugin/events/notification_test.go. This
// block re-pins the notification event contract: construction, default
// timestamp, the lowercase SSE wire keys (id/type/severity/message/
// provider/model per web contract commit 3cce511f), and the type /
// severity constant catalog. Colocated with events_test.go per the
// one-test-file-per-source rule.
