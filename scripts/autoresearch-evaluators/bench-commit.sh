...
if [ -n "$bench_output_fixture" ]; then
  if [ ! -f "$bench_output_fixture" ]; then
    echo "bench-commit.sh: FLOWSTATE_AUTORESEARCH_BENCH_OUTPUT=$bench_output_fixture not found" >&2
    exit 65
  fi
  bench_output=$(cat "$bench_output_fixture")
else
  if ! command -v go >/dev/null 2>&1; then
    echo "bench-commit.sh: 'go' not found on PATH" >&2
    exit 64
  fi
  if ! command -v awk >/dev/null 2>&1; then
    echo "bench-commit.sh: 'awk' not found on PATH" >&2
    exit 64
  fi
...
if [ -z "$ns_per_op" ]; then
  echo "bench-commit.sh: no ns/op line found in bench output" >&2
  printf '%s\n' "$bench_output" >&2
  exit 65
fi
...
if [ "$ns_per_op_int" -le 0 ] 2>/dev/null; then
  echo "bench-commit.sh: parsed ns/op '$ns_per_op' is zero or negative" >&2
  exit 66
fi
...
case "$bench_metric" in
  ...
  ns_per_op)
    ...
    ;;
  *)
    echo "bench-commit.sh: unrecognised FLOWSTATE_AUTORESEARCH_BENCH_METRIC=$bench_metric" >&2
    exit 67
    ;;
esac
